...
{
    // This test identifies implementation-defined behavior.
    // As the standard does not specify whether plain char is signed or unsigned,
    // a char on AArch64 is expected to be unsigned whereas a char on x86_64 is expected to be signed.
#if defined(__aarch64__)
    static_assert(std::numeric_limits<char>::is_signed == false,
                  "plain 'char' type is expected to be unsigned on AArch64");
#elif defined(__x86_64__)
    static_assert(std::numeric_limits<char>::is_signed == true, "plain 'char' type is expected to be signed on x86_64");
#else
#error "Unsupported platform: neither aarch64 nor x86_64"
#endif
